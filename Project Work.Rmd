...
We want to make a maximum-likelihood estimator for $\psi$, $\widehat{\psi^{mle}}$, given the observed data. Let us start by writing the likelihood function.

We use the fact that $\psi = \frac{1-2\pi}{1-\pi}$, where we can rearrange terms:
...
For our observed data, we will use $t$ as defined by $T\sim Binom(170,\pi)$ where $\pi=\frac{\pi_v}{\pi_v+\pi_p}$, which represents the number of patients who contracted COVID who had initially been in the vaccinated group rather than the placebo one. So the likelihood function is as follows: 

\begin{align*}
L(\psi) &= f(t | \psi) \\
&= {170\choose t} \pi^t \cdot (1 - \pi)^{170-t} \\
