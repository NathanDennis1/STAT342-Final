...
COVID-19 (SARS-CoV-2), is an infection which resulted in a global pandemic that negatively affected millions worldwide. Due to it's widespread impact, effective vaccines are necessary.

### Methods

A brief placebo-controlled experiment was conducted
...
First, define the following variables for simplicity:\
...
Recall the distribution of T well approximated by binomial for the BNT162b2 group study. Therefore:\
...
From here, the parameter of interest is the vaccine efficacy $\psi$, which is written as $\psi=\frac{\pi_p- \pi_v}{\pi_p}$. We can re-write this in terms of $\pi$ by first finding $\pi_v$
\begin{align*}
\pi &= \frac{\pi_v}{\pi_v+\pi_p} \\
\pi(\pi_v+\pi_p) &= \pi_v \\
...
With this definition, we can construct the hypothesis of interest for $\psi$. The FDA requires at least 30% efficacy for a new therapy, in this case the BNT162b2 vaccine, to be approved. We can operate under the null hypothesis, $H_0: \psi=0.3$, where the alternative is $H_1: \psi > 0.3$

Within this section the following rearrangement of terms is used, given $\psi = \frac{1-2\pi}{1-\pi}$
\begin{align*}
...
Using this posterior distribution we can find the 95% Bayesian credible interval for $\pi_0$, where we transform this for the vaccine efficacy, $\psi = \frac{1-2\pi}{1-\pi}$, and calculate a p-value to test the null hypothesis. We use a 95% interval similar to that used in the previous study. Building off the credible interval we can also calculate the posterior median for $\pi$ in order to obtain the posterior median for $\psi$, finding the 0.5 percentile for the posterior distribution for $\pi$.
...
\end{align*}
We know that t cannot be a negative number and that $t \leq 170$ since there are 170 positive cases, the number of individuals in the vaccine group in these 170 cases must be less than 170 but greater than 0. So, the corresponding fractions $\frac{1}{170}-\frac{1}{t} \leq 0$ and $\frac{d^2}{d\psi^2} \ell(\hat{\psi_0}^{mle}) < 0$, so the critical point and hence MLE is a maximum.\
Next we want to construct a 95% Wald confidence interval for $\psi$, so we first compute the standard error which is defined to be $\hat{SE}(\hat{\psi_0}^{mle}) = \frac{1}{\sqrt{-\ell''(\hat{\psi_0}^{mle})}} = \frac{1}{\sqrt{\left(\frac{1}{170}-\frac{1}{t} \right)(t-170)^2}}$, where again we know t=8 in the dataset from Table 1 and we can find this in the results.
...
## Prior 1
We calculate the 95% credible interval for $\pi_0$, [0.023194, 0.08799], and the posterior median as $\pi_0 = 0.048932$. We can convert this to create an interval for $\psi$ using the fact that $\psi = \frac{1-2\pi}{1-\pi}$, plugging in the credible interval values. We obtained a 95% Bayesian credible interval reported as [90.3%, 97.6%].

We can also calculate a p-value for this to confirm our beliefs. As a reminder, $H_0: \psi_0<=0.3$ and $H_1: \psi_0>0.3$, where the p-value was calculated to be $1.960014 \times 10^{-28}$, which is nearly 0.
...
Using R software we calculate the 95% credible interval for $\pi_0$ to be [0.257257, 0.348361]. We again convert this to create an interval for $\psi$ using the equality $\psi = \frac{1-2\pi}{1-\pi}$. So our 95\% interval for $\psi$ is (0.465409, 0.65364). This interval is above and does not contain the value of 0.3, so we conclude the vaccine efficacy rate is greater than 30\%.

Again we can also calculate a p-value under the hypothesis: $H_0: \psi_0<=0.3$ and $H_1: \psi_0>0.3$, where the p-value was calculated to be $6.306123 \times 10^{-8}$, which is nearly 0 and we safely reject the null in favor of the alternative.

Furthermore, we calculate the posterior median as $\pi_0 = 0.274467$. We the see that the posterior median for $\psi$ comes out to be 0.621702, which is extremely large.




## Prior 3
Using R software we calculate the 95% credible interval for $\pi_0$ to be [0.022248, 0.088797]. We can convert this to create an interval for $\psi$ using the fact that $\psi = \frac{1-2\pi}{1-\pi}$, plugging in the credible interval values. We obtained a 95% Bayesian credible interval reported as [0.90255, 0.977246]. This interval does not contain the value 30% and is much larger than this, so we conclude the vaccine efficacy is larger than 30%.

We can also calculate a p-value for this unswe $H_0: \psi_0<=0.3$ and $H_1: \psi_0>0.3$, where the p-value was calculated to be $2.181923 \times 10^{-21}$, which is nearly 0 and we safely reject the null in favor of the alternative.
...
#w_obs <- 2*(8*log(1-mle_psi)/(2-mle_psi))+162*log(1/(2-mle_psi))-
...
We use the derivation for the maximum likelihood for $\psi$, with t=8 from Table 1, to get: $\hat{\psi_0}^{mle} = \frac{170-2t}{170-t}= \frac{170-2\cdot8}{170-8} = \frac{77}{81}$. We showed that the second derivative yield a negative value, so this is a maximum. We see that this value is very large, as the estimated vaccine effiacy rate is around 0.950617. The MLE allows us to estimate the parameters of our model which maximimizes the likelihood function, we have a very large estimate.

We then construct a 95% Wald Confidence interval around this estimator, which was [0.915563, 0.985672]. This helps provide an estimate for the range of our parameter, a range of values within the true parameter. We are 95% confident that the true parameter, $\psi$ falls in the range [0.9045477, 0.9966868].

Furthermore, we can calculate the w-statistic from the likelihood ratio test. Large values of W are known to provide evidence in favor of the alternative, where we obtained a value of 
...
lower_ci <- mle_psi - qnorm(0.995) * psi_se
upper_ci <- mle_psi + qnorm(0.995) * psi_se
