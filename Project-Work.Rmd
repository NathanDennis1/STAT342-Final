...
The bootstrap simulation will take 10,000 samples of $\bar{X}$ and $\bar{Y}$ from $\sum_{X} \sim \mbox{Binom}(17411, 8/17411)$, $\sum_{Y} \sim \mbox{Binom}(17511, 162/17511)$.
...
```{r label ="bootstrap", include=F, cache = TRUE}
# Requirements
set.seed(342)
...
The Bootstrapped data was as follows:
```{r figure 4.6.2, echo = F}
p1
```

A more detailed view and checking for normality:
```{r figure 4.6.1, echo = F}
...
Finally, our bootstrapped confidence interval using the quantile method for $\psi$ found that we are 95\% confident that the true vaccine efficacy is between [0.9110567, 0.9817138], with our median bootstrapped $\psi = 0.95145$. Our entire confidence interval lies above 30\% so there is sufficient statistical evidence to conclude that the vaccine is sufficiently effective. Therefore, we can safely reject the null hypothesis.
